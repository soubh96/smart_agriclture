...
def getfrom_sensorfour():
  ...
  print("Temperature detected on sensor No.4 =",temperature,"Degree Celsius")
  print("Humidity of detected on sensor No.4 =",humidity,"%")
...
def send_sensorfourdata_func(send_sensorfour_data):
    while 1:
      ser1.write(send_sensorfour_data.encode('utf-8')+b"\r\n")
      time.sleep(0.5)
      sensorfour_response=ser1.readline().decode('utf-8').strip()
      print("sensorfour_Response-->",sensorfour_response)
      time.sleep(0.5)
      sensorfour1_response=ser1.readline().decode('utf-8').strip()
      print("sensorfour1_Response--->",sensorfour1_response)
      if sensorfour_response == "+SEND: OK":
        time.sleep(0.5)
        break;
...
def utility_func():
    ...
    time.sleep(0.4)
    sensorfour_temp,sensorfour_hum=getfrom_sensorfour() 
    ...
    send_sensorfour_data=f"AT+send=\"Tempsensor4-->{sensorfour_temp}--Humiditysensor4-->{sensorfour_hum}\""   
   ...
   # time.sleep(0.2)
    ...
    send_alldata_func(send_alldata_command)
    time.sleep(0.2)
    send_sensorfourdata_func(send_sensorfour_data)
